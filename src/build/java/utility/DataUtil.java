...
public class DataUtil {
    ...
    */
    ...
    private void processDataUtility(String[] args) {
        ...
        if (args[0].equals("executeBuild")) {
            executeBuild(args);
        } else if (args[0].equals("extractLogs")) {
            extractLogs(args,con1);
        } else if (args[0].equals("getSandboxName")) {
    ...
    public void executeBuild(final String[] args) {
        ...
        try {
            ...
            final StringEntity body = new StringEntity(packageXMLs.get(0));
    ...
    */
    ...
    public String retrieveEnhancement(AppConnection con, final String enhName) {
        ...
        final String uri = con.getBaseURI() + "query/?q=SELECT+"+SOQL_fields+"+FROM+Application_Request__c+where+Deploy_Branch__c='"+enhName+"'";
    ...
    // Query environment data using REST HttpGet
    public String extractLogs(String[] args, AppConnection con) {
        System.out.println("\n_______________ Extract Log _______________");
        ...
        */
        String inputDay = args[1];
        String inputDirectory = args[2];
        if (DEBUG_ON) System.out.println("100-extractLogs inputDay: " + inputDay+"\n##creating base log directory: "+processFile ("directory","",inputDirectory,"") );
        //final String uri = con.getBaseURI() + "?q=SELECT+"+SOQL_fields+"+FROM+Application_Request__c+where+Deploy_Branch__c='"+enhName+"'";
        final String uri = con.getBaseURI() + "query/?q=SELECT+Id,EventType,LogDate+From+EventLogFile+Where+LogDate="+inputDay+"+order+by+LogDate";
        //final String uri = con.getBaseURI() + "?q=SELECT+"+SOQL_fields+"+FROM+Application_Request__c";
        ...
        //final String uri = baseUri + "/getEnvironmentDetails?envName="+envName;
        if (DEBUG_ON) System.out.println("100.5-extractLogs URL: " + uri);
        ...
        try {
            ...
            final HttpGet httpGet = new HttpGet(uri);
            if (DEBUG_ON) System.out.println("101-extractLogs-GET: oauthHeader2: " + con.getOauthHeader());
            httpGet.addHeader(con.getOauthHeader());
            httpGet.addHeader(prettyPrintHeader);
            ...
            // Process the result
            final int statusCode = response.getStatusLine().getStatusCode();
            if ( DEBUG_ON ) System.out.println("101-extractLogs statusCode: " + statusCode+" response: "+response);
            if (statusCode == 200) {
                try {
                    final String response_string = EntityUtils.toString(response.getEntity());
                    //if ( DEBUG_ON ) System.out.println("103.1-extractLogs response_string: " + response_string);
                    //final String escapedString = StringEscapeUtils.unescapeJava(response_string.substring(response_string.indexOf('{'),response_string.length()-1));
                    //final JSONObject obj = new JSONObject(escapedString);
                    //JSONParser parser = new JSONParser(); 
                    //JSONObject json = (JSONObject) parser.parse(response_string);
                    ...
                    // typecasting obj to JSONObject 
                    JSONObject jo = (JSONObject) obj; 
                    //if ( DEBUG_ON ) System.out.println("103.2-extractLogs JSONObject: " + jo);

                    JSONArray ja = (JSONArray) jo.get("records"); 
                    //if ( DEBUG_ON ) System.out.println("103.3-extractLogs JSONArray: " + ja);
                    ...
                    { 
                        ...
                        while (itr1.hasNext()) { 
                            Map.Entry pair = itr1.next();
                            //if ( DEBUG_ON ) System.out.println("103.4-extractLogs pair: " + pair);
                            workStr1 = (String) pair.getKey(); 
                            //if ( DEBUG_ON ) System.out.println("103.5-extractLogs workStr1: " + workStr1);
                            if ( workStr1.equals("Id")) {
                                //if ( DEBUG_ON ) System.out.println("103.6-extractLogs workStr1:value " + pair.getValue());
                                workStr2 = (String)pair.getValue();
                            } else if (workStr1.equals("LogDate")) {
                                //if ( DEBUG_ON ) System.out.println("103.7-extractLogs workStr3:value " + ((String)pair.getValue()).split(":")[0]+"* break value: "+breakValue+"*");
                                workStr3 = ((String)pair.getValue()).split(":")[0];
                                if ( !breakValue.equals(workStr3) ) {
                                    breakValue = workStr3;
                                    if ( !processFile ("directory",inputDirectory,breakValue,"")){//}.split(":")[0],"") ) {
                                        System.out.println("########Error creating directory: "+breakValue);//.split(":")[0]);
                                    }
                                } 
                                System.out.println(""+(++cnt)+"-Id: "+workStr2+" directory: "+breakValue);//.split(":")[0]);
                                //downloadLog(workStr2,workStr3,inputDirectory+"/"+workStr3,con1);
                            }
                        } 
                    }     
                    //if ( DEBUG_ON ) System.out.println("103.6-extractLogs single Id: " + workStr2+" logDate: "+workStr3);//.split(":")[0]);
                } catch (final ParseException je) {
                    je.printStackTrace();
               }
            } else {
    ...
    // Query environment data using REST HttpGet
    public String downloadLog(String logId,String logDate,String logDirectory,AppConnection con) {
        System.out.println("\n_______________ Extract Log _______________");

        /*
        #set elfs to the result of ELF query
        elfs=`curl https://${instance}.salesforce.com/services/data/v32.0/queryquery/?q=Select+Id+,+EventType+,+LogDate+From+EventLogFile+Where+LogDate+=+${day} -H "Authorization: Bearer ${access_token}" -H "X-PrettyPrint:1"`
        https://${instance}.salesforce.com/services/data/v32.0/sobjects/EventLogFile/${ids[$i]}/LogFile
        https://va.my.salesforce.com/services/data/v47.0/query/

        */
        if (DEBUG_ON) System.out.println("200-downloadLog inputDay: " + logId);
        //final String uri = con.getBaseURI() + "?q=SELECT+Id,EventType,LogDate+From+EventLogFile+Where+LogDate="+logId;
        final String uri = con.getBaseURI() + "sobjects/EventLogFile/"+logId+"/LogFile";

        String returnMessage = "";//" \"enhancement\" : [ {";

        //final String uri = baseUri + "/getEnvironmentDetails?envName="+envName;
        if (DEBUG_ON) System.out.println("200.5-downloadLog URL: " + uri);
        ...
        try {
            ...
            final HttpGet httpGet = new HttpGet(uri);
            if (DEBUG_ON) System.out.println("201.1-downloadLog-GET: oauthHeader2: " + con.getOauthHeader());
            httpGet.addHeader(con.getOauthHeader());
            httpGet.addHeader(prettyPrintHeader);
            ...
            // Process the result
            final int statusCode = response.getStatusLine().getStatusCode();
            if ( DEBUG_ON ) System.out.println("201.2-downloadLog statusCode: " + statusCode);
            if (statusCode == 200) {
                try {
                    final String response_string = EntityUtils.toString(response.getEntity());
                    if ( DEBUG_ON ) System.out.println("201.3-downloadLog preparing to write to file");
                    if ( !processFile ("file",logDirectory.split(":")[0],logId,response_string) ) {
                        System.out.println("########Error creating ouput Log: "+logDirectory+"/"+logId);
                    } else {
                        if ( DEBUG_ON ) System.out.println("201.4-returned from writing file");
                    }
        ...
        if ( DEBUG_ON ) System.out.println("201.9-exiting processFile");
        return returnMessage+"} ],";
    }

    public boolean processFile (String fileOpt, String pathName, String inputName, String contentName ) {

        boolean returnResult = false;

        String wStr1 = "";

        //if ( DEBUG_ON ) System.out.println("301.1-inside processFile: "+inputName);
        if ( fileOpt.equals("directory") ) {
            //System.out.println("Enter the path to create a directory: "+pathName);
            //System.out.println("Enter the name of the desired a directory: "+inputName);

            //Creating a File object
            //File file = new File(pathName+"/"+inputName);
            wStr1 = pathName+"/"+inputName.split(":")[0];
            //File file = new File(inputName.split(":")[0]);
            File dir = new File(wStr1);
            try {
                Files.createDirectory(dir.toPath());
                returnResult=true;
            } catch (IOException ioe) {
                returnResult=false;
                //System.out.println("#Sorry couldn’t create specified directory ("+wStr1+"): "+ioe.getMessage());//pathName+"/"+inputName+")!");
            }
            //Creating the directory
            //returnResult = file.mkdir();

            if(returnResult){
               //System.out.println("##Directory created successfully ("+inputName.split(":")[0]);//pathName+"/"+inputName+")!");
            }else{
               System.out.println("###Sorry couldn’t create specified directory ("+inputName.split(":")[0]);//pathName+"/"+inputName+")!");
            }

        } else if ( fileOpt.equals("file")) {

            try {
                FileWriter myWriter = new FileWriter(pathName+"/"+inputName+".csv");
                myWriter.write(contentName);
                myWriter.close();
